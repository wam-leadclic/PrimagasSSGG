...
import getOrders from '@salesforce/apex/PedidosDeGasPageController.getOrders';
...
const COLUMNS = [
    { label: 'Código', fieldName: 'Codigo__c',cellAttributes: { alignment: 'left' },sortable: true },
    ...
    { label: 'Estado', fieldName: 'Status__c',sortable: true },
];
const NO_RECORDS_MESSAGE = 'No hay pedidos de gas asociados a esta dirección de entrega.';
const NO_TYPE_MESSAGE ='Esta dirección de entrega no tiene un tipo asignado';
const INVALID_ADDRESS_MESSAGE = 'Este tipo de dirección de entrega no puede tener pedidos asociados';
const NO_EXTERNAL_ID_MESSAGE = 'Falta el siguiente id de Navision: ';
const LOADING_MESSAGE = 'Solicitando datos...';
const VALID_ADDRESS = ['BULK','CT','LNG'];
export default class GasOrders extends LightningElement {
    @api recordId
    ...
    callApex() {
        ...
        getOrders({recordId : this.recordId}).then(result => {
            if (result.hasError) {
